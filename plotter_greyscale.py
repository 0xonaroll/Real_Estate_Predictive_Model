...
# img_flist_train = '../data/flist/train_bucket'
# img_flist_test = '../data/flist/test_bucket'
# ckpt_file = ''
output_dir = '../data/results'
# train_log = 'train_log.txt'
# test_log = output_dir + '/' + model_name + '_new_test_log.txt'
...
# python3 plotter.py convnet_median1
# python3 plotter.py convnet_log1
# python3 plotter.py convnet_median_greyscale1
# python3 plotter.py convnet_bucket1
...
class ImageFilelist(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if model_name == 'convnet_median_greyscale1':
            # print(model_name, '11111')
            return torch.mean(img, 0, keepdim=True), target
        else:
            # print(model_name, '22222')
            return img, target
...
def test(model, ep, file_log):
    ...
    with torch.no_grad():
        ...
        print('Train Set Epoch {} Total Loss on {} images: {}'.format(str(ep), total_ct, total_step),
        ...
        print('Test Set Epoch {} Total Loss on {} images: {}'.format(str(ep), total_ct, total_step),
              file=open(file_log, 'a+'))
    # model.train()
...
for ep in range(5, 51, 5):
    load_ckpt = '../data/results/{}_model_{}.ckpt'.format(model_name, str(ep))
    model = ConvNet(num_classes).to(device)
    model.load_state_dict(torch.load(load_ckpt))
    # print(load_ckpt, model)
    model.eval()
    criterion = nn.MSELoss()
